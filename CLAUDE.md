...
- **Second provider — اقامت۲۴ / GRS** (`providers/eghamat24/`, spec: GRS Agency API v1.18.4). Nothing like هتل‌یار: auth is a fixed `Client-Token` header (no login, no session), it is real REST (GET reads with query params, POST writes, meaningful HTTP status — 403 bad token, 422 validation), and prices are **rial**, converted in `eghamat24.mapper.ts`. What it shares is the trap: an inventory error still comes back as **HTTP 200** with only `errors[]` filled, so both the status code and `errors` must be checked. `GRS_MODE=live` picks `GrsHttpClient` (needs `GRS_URL` + `GRS_TOKEN`), anything else falls back to `GrsMockClient` — same contract as `GDS_MODE`, same reason. Test env is `https://hotel-test-01.denv.ir`, **reachable from Iran only**; verify with `pnpm grs:check` (read-only, no booking). ⚠ **Only `GET /v1/cities` and `POST /v1/book` are confirmed against the doc** — every other path in `ENDPOINTS` (`grs-http.client.ts`) is inferred from the service name and marked `?`; the first `grs:check` run from Iran is what pins them down, and it probes alternate spellings on a 404 so one run settles all of them. See `docs/eghamat24/webservice.md`.
